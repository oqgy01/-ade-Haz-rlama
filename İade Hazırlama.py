...
order_url = "https://task.haydigiy.com/FaprikaReturnXls/30WGD6/1/"
...
def get_specific_order_data(order_id):
    order_url = f"https://task.haydigiy.com/admin/order/edit/{order_id}/"
    ...
    # "Mobil Uygulama İndirme Kuponu" kontrolü
    mobil_indirme_kuponu = order_soup.find(string="Mobil Uygulamada 5 Dakika Geçir İndirimi")

    if mobil_indirme_kuponu:
        data["İndirim Kuponu"] = "Mobil Uygulamada 5 Dakika Geçir İndirimi"
    else:
        data["İndirim Kuponu"] = "Yok"
...
try:
    df_sonuc = pd.merge(df_kopya, df_kaynak[['SiparisId', 'Vade Farkı:']], on='SiparisId', how='left')

    # "2. Sutun" sütununu "iadeler kopya.xlsx" dosyasındaki ilgili sütuna yaz
    df_kopya['Vade Farkı:'] = df_sonuc['Vade Farkı:']
...
try:
    df_sonuc = pd.merge(df_kopya, df_kaynak[['SiparisId', 'İndirim Kuponu']], on='SiparisId', how='left')

    # "2. Sutun" sütununu "iadeler kopya.xlsx" dosyasındaki ilgili sütuna yaz
    df_kopya['İndirim Kuponu'] = df_sonuc['İndirim Kuponu']
